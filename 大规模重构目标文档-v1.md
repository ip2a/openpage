...
## 2026-07-23：Rust-first 门面最终收口

当前仓库已完成并验证以下目标：

- Rust 核心、PyO3 绑定和 Python 门面统一使用 `Browser`、`Page`、`Session` 核心模型；
- 删除旧复合页面模型、旧查询别名、兼容导入、回退模块和废弃测试入口；
- Python wheel 可以构建，并在独立临时 uv 环境中导入 `openpage` 与 `openpage.openpage_rs`；
- 公共测试脚本不再引用已删除的 Python 兼容测试；
- Rust workspace、CLI、MCP smoke、Python 门面测试和发布元数据检查已通过；
- 工作树干净，未将 `target/`、`dist/`、虚拟环境、wheel 或 Python 缓存纳入 Git。

本地已验证范围不等同于未提供的远端多平台 CI runner 结果；对于 Windows、Linux ARM 和双架构 macOS，仍以实际发布环境的 runner 结果为准，不以本机结果冒充。
