...
export function RecordingConsole() {
  ...
  const connect = async () => {
    try {
      setError("");
      await invoke("ensure_browser", { session });
      await refresh();
    } catch (value) {
      setError(String(value));
    }
  ...
  const updateLocator = (index: number, locator: string) => {
    setFlow((current) => ({
      ...
      steps: current.steps.map((step, stepIndex) =>
        stepIndex === index ? { ...step, target: { ...step.target, locator } } : step,
      ),
    }));
  };
  ...
  return (
    <main className="recording-console">
      <header>
        <div>
          <span className="eyebrow">OPENPAGE</span>
          <h1>录制控制台</h1>
        </div>
        <span className={status.recording ? "badge live" : "badge"}>
          {status.recording ? "录制中" : "已停止"}
        </span>
      </header>
      <section className="session">
        <label>
          Session <input value={session} onChange={(event) => setSession(event.target.value)} />
        </label>
        <span className="current-url" title={url}>
          {url || "未连接"}
        </span>
      </section>
      <section className="toolbar">
        <button onClick={() => void connect()}>启动/连接浏览器</button>
        <button className="primary" onClick={() => void run("recorder.start")} disabled={status.recording}>
          开始录制
        </button>
        <button onClick={() => void run("recorder.stop")} disabled={!status.recording}>
          停止录制
        </button>
        <button onClick={() => void run("recorder.replay")} disabled={!flow.steps.length}>
          回放
        </button>
        <button onClick={() => saveFlowFile(flow)}>保存 JSON</button>
        <button
          onClick={() =>
            openFlowFile().then((next) => {
      ...
      <section className="summary">
        <strong>{flow.steps.length}</strong>
        <span>个步骤</span>
          ...
          <li key={index}>
            ...
            <div className="step-body">
              {step.target && (
                <input
                  className="locator"
                  value={step.target.locator ?? ""}
                  onChange={(event) => updateLocator(index, event.target.value)}
                />
              )}
            ...
            <div className="step-actions">
              ...
              <button onClick={() => move(index, 1)} disabled={index === flow.steps.length - 1}>
                ↓
              </button>
