...
- Browser download configuration now follows the same Rust-first rule too: Python passes a download path into Rust, and Rust applies `Browser.setDownloadBehavior` directly through CDP.
...
- Browser download-path configuration and a local file-download flow are now verified from Python against the Rust core, and `run_checks.sh` rebuilds the local extension before Python checks so verifier results cannot accidentally come from stale Rust artifacts.
